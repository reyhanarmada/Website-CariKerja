...
import { companyTypeDefs } from './schema/company.js';
...
const server = new ApolloServer({
  schema: buildSubgraphSchema({
    typeDefs: parse(companyTypeDefs),
    ...
    context: async ({ req }) => {
      ...
      if (token) {
        try {
          ...
          console.warn('Invalid token in company-service context:', err.message);
        }
      }
      return { token, user };
    },
...
console.log(`🚀 company-service running at http://localhost:${PORT}/graphql`);
